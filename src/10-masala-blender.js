...
export function createRecipe(steps) {
  ...
  const stepsName = { grind, roast, pack }
  const pipe = steps.map(name => stepsName[name]).filter(fn => typeof fn === 'function')

  if (pipe.length === 0) return x => x

  return function (input) {
    return pipe.reduce((acc, fn) => fn(acc), input)
  }
}
